    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < (1 << k); j++) {
            ...
            if (i > 0) {
                ...
                currDp[0][1] = (1 << k) - 1;
                if (j & (1 << 0) && currDp[0][0] < matrix[0][i] + matrix[0][i - 1] + dp[i - 1][((1 << k) - 1) & ~(1 << 0)]) { //
                    currDp[0][0] = matrix[0][i] + matrix[0][i - 1] + dp[i - 1][((1 << k) - 1) & ~(1 << 0)];
                    currDp[0][1] = ((1 << k) - 1) & ~(1 << 0);
                }
                currDp[1] = currDp[0];
                if (j & (1 << 1) && j & (1 << 0) && currDp[1][0] < matrix[1][i] + matrix[0][i] + dp[i - 1][(1 << k) - 1]) {
                    currDp[1][0] = matrix[1][i] + matrix[0][i] + dp[i - 1][(1 << k) - 1];
                    currDp[1][1] = (1 << k) - 1;
                }
                ...
                currDp[1][0] = std::max(0, matrix[0][0] + matrix[1][0]);
            }
